...
- JDK버전과 Spring Boot Initializr에 선택하는 JDK버전이 일치해야 함
...
- Maven
  ...
  6. VS Code 확장 중 Maven Dependency Explorer
...
- 내용 추가
- Spring Boot Banner Generator 사이트
...
- 각 폴더 구조
  ...
  - com.lsh.spring03 : 패키지. 폴더로 구성
    - HelloController 클래스에 접근하려면
    - com.lsh.spring03.controller.HelloController 접근해야 함
  - Spring03.Application.java : 시작프로그램.
  - src/mian/resources : 자바파일 이외 HTML, CSS, JS, 환경파일 등 리소스파일 저장되는 위치
  ...
  - `build.gradle` : 그레이들 환경 파일. Groovy 기반으로한 빌드도구. dependencies만 잘 구성하면 됨
...
- 컴포넌트 구체화해서 해달클래스 IoC컨테이너 Bean으로 등록
...
- Spring Boot JPA + Oracle + Thymeleaf + React
  ...
  - JPA - DB설계 없이 엔티티 클래스 만으로 테이블을 자동 생성해주는 기술. SQL도 필요없음.
    - JPA 이전 - MyBatis( SQL + XML로 구성된 ORM(Object Relational Mapping) 기술)
  - H2 - Oracle, MySQL등과 달리 인메모리DB. Spring Boot에서 자동으로 실행해주는 DB
...
- Spring Boot 프로젝트 생성
  ...
  1. 명령 팔레트로 시작(Ctrl + Shift + P) : Spring Initializr: Craete a Gradle Project
  ...
  8. Dependency
     ...
     - Spring Web : 프론트엔드(html) 개발을 할때 필요한 의존성
     - Thymeleaf : html + Spring Boot 태그 매핑을 해주는 기능
     - H2 Database : 개발동안 필요한 인메모리DB
     - Oracle Driver : 실제 운영할 DB
     - Spring Data JPA : DB 생성 + ORM
  9. 저장위치 지정, Generate into this folder 선택
  10. 오른쪽 하단 프로젝트 폴더 Open 클릭
...
6. http://localhost:9097/h2-console 접속
   <img src="image\sb012.png" width="450">

7. application.properties에 JPA 설정
   ...
   - spring.jpa.hibernate.ddl-auto 종류
     - create : SB서버 시작시 테이블을 모두 삭제 후 재생성(데이터 모두 휘발)
...
8. MVC 패턴에 맞춰 각 기능별로 패키지(폴더) 생성

   1. controller, entitiy, repository ,service...
   2. entity
...
12. 단위테스트
    ...
    3. test/.../backboard/BackboardApplicationTests.java에 단위테스트 메서드 작성
       <img src="image\sb013.png" width="450">
    4. SELECT, SELECT ... WHERE 단위테스트
    5. 디버그콘솔에서 쿼리 로그로 확인. application.properties 추가
...
2. 계발 계속
   ...
   3. @(Annotation)추가
      ...
      - @OneToMany(mappedBy = "board", cascade = CascadeType.REMOVE) : 1대다 ERD 관계에서 부모클래스에 작성하는 부분
      - @ManyToOne : 다대1 ERD관계로 자식클래스에 작성하는 부분
      - @Service : 서비스 모듛을 지칭(SpringFramework)
   ...
   10. 상세 페이지에 댓글 저장 기능 추가
       ...
       - ReplyServixe, ReplyController 각각 생성, 작성
...
5. 전체 HTML에 디자인 적용
   ...
   1. build.gradle에 thymeleaf-layout 의존성 추가
      ...
      ```gradle
      // Thymeleaf layout 의존성 추가. 250701. 신규추가. 이성훈
   ...
   https://github.com/user-attachments/assets/42b6f7b1-5020-4285-8360-f6e936e70124
...
1. DB연동 개발
   ...
   5. board_create.html에 입력검증 태그, 속성 등 추가
      ...
      - GetMehod, PostMethod에 BoardForm 파라미터를 추가!
...
3. Bootstrap Navigation 구현
   - templates/layout.html 네비게이션 태그 구현
...
1. 게시판작업
   ...
   3. 게시글 개수만큼 번호가 나오도록 수정
      ...
      2. 게시물번호 = 전체 게시물수 - (현재페이지번호 * 10[페이지당 게시글 수]) -페이지당 순차 인덱스
      3. board_list.html의 `<td th:text="${index.count}"></td>`수정
 
   4. Bootstrap 배지로 각 게시글마다 댓글개수 표시
      - MyBatis로 작업된 Spring Boot : 쿼리변경, 도메인변경, 엔티티변경, html까지 세군데 변경
