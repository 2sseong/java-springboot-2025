...
public class SecurityConfig {
    ...
    SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.authorizeHttpRequests((ahr) -> ahr.requestMatchers("/**") // 인증되지 않은 모든 페이지 요청을
                                                                       // 허락(로그인창 안뜸)
                .permitAll()
                .anyRequest()
                .authenticated())
                ...
                .headers((hdr) -> hdr.addHeaderWriter(new XFrameOptionsHeaderWriter(
                        XFrameOptionsHeaderWriter.XFrameOptionsMode.SAMEORIGIN // h2-console이 Frame방식(구시대방식)으로
                )));
